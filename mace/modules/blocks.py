...
class AOInteractionBlock(torch.nn.Module):
    def __init__(
        ...
        hidden_irreps: o3.Irreps,
        avg_num_neighbors: float,
        edge_irreps: Optional[o3.Irreps] = None, # If None is node_feats_irreps
        radial_MLP: Optional[List[int]] = None,
...
class AORealAgnosticResidualInteractionBlock(AOInteractionBlock):
    def _setup(self) -> None:
        ...
        ao_input_dim = self.ao_feats_irreps.num_irreps
        self.ao_conv_tp_weights = nn.FullyConnectedNet(
            [ao_input_dim] + self.ao_MLP + [self.conv_tp.weight_numel],
            torch.nn.functional.silu,  # gate
        )
